...
from Gold-layer statistics using the Groq API.
...
LEAGUE_AVG = {
    ...
    "boundary_rate_allowed": 0.078,
    "avg_expected_runs_per_ball_allowed":   0.312,
}

# ─── Prompt Templates ───────────────────────────────────────────────────────────

PITCHER_INSIGHT_PROMPT = """\
You are an elite cricket analyst evaluating T20 or ODI performances for an international franchise coaching staff.

Bowler: {pitcher_name}
Date: {game_date}
Opponent: {opponent}

Performance data (this match):
  Deliveries bowled: {total_pitches}
  Avg pace:          {avg_velo} kph  (season avg: {season_avg_velo} kph, delta: {velo_delta:+.1f})
  Play & miss rate:  {play_and_miss_rate:.1%}  (league avg: 25.4%)
  Dot Ball %:        {dot_ball_percentage:.1%}  (league avg: 28.1%)
  Stump line rate:   {stump_line_rate:.1%}
  Wide delivery rate:{wide_delivery_rate:.1%}
  Expected runs/ball:{avg_expected_runs_per_ball_allowed:.3f}  (league avg: .312)
  Boundary rate:     {boundary_rate_allowed:.1%}
  Variation mix:     {variation_diversity:.3f}  (higher = more varied deliveries)
  Run value delta:   {total_re_delta:+.2f} runs

Evaluate the bowler using cricket terminology (e.g. line and length, swing, economy rate indicators).
...
{{
  ...
  "key_finding": "<2-3 sentences: most important statistical story regarding line and length, variations, or pace>",
  "concern_flag": "<null or one sentence on a meaningful red flag (e.g. boundary rate, wide deliveries)>",
  "pitch_mix_note": "<one sentence on variation usage, swing, or pace trend if notable, else null>"
}}
"""

ANOMALY_INSIGHT_PROMPT = """\
You are an elite cricket data scientist reviewing statistical anomalies from today's T20 and ODI matches.

The following bowler-match metrics are outliers (Z-score > 2.0 vs. that bowler's 30-day baseline):

{anomaly_list}

Select the single most analytically interesting anomaly and explain it in 2-3 sentences.
Consider potential causes: fatigue, variation mix change, opposing batsman quality, release point shift, pitch conditions (swing/spin/seam), or sample size caveats. Be precise — cite the actual numbers.
...
GAME_SUMMARY_PROMPT = """\
You are an elite cricket analyst writing a post-match intelligence brief for the coaching staff.

Match: {home_team} vs {away_team}, {game_date}
Final: {home_score} - {away_score}

Top performers (bowlers by Dot Ball %):
{pitcher_table}

Top performers (batsmen by expected runs/ball):
{batter_table}

Write a 3-4 sentence match intelligence summary. Focus on the most statistically significant performance, what it means in context, and one forward-looking scouting observation.
...
class LLMClient:
    """Thin wrapper around Groq client with retry + rate limiting."""

    def __init__(self) -> None:
        self.client = groq.Groq()   # reads GROQ_API_KEY from env
...
class InsightWriter:
    ...
    def enrich_game(self, game_pk: int, game_date: str) -> list[InsightResult]:
        """Generate LLM insights for all bowlers in a given game."""
        rows = self.con.execute("""
            ...
            ORDER BY pgs.dot_ball_percentage DESC
    ...
    def detect_anomalies(self, game_date: str) -> dict[str, Any] | None:
        ...
        anomalies = self.con.execute("""
            ...
            WHERE game_date = ?
              AND ABS(play_and_miss_rate - rolling_30d_whiff_rate) > 0.08
              AND total_pitches >= 40   -- meaningful sample
            ORDER BY ABS(play_and_miss_rate - rolling_30d_whiff_rate) DESC
            LIMIT 5
        """, [game_date]).df()
        ...
        anomaly_text = "\n".join([
            f"- {r.pitcher_name}: play_and_miss_rate {r.play_and_miss_rate:.1%} "
            f"(30d avg: {r.rolling_30d_whiff_rate:.1%}, "
            f"velo delta: {r.pace_vs_30d_avg:+.1f} mph)"
    ...
    def _generate_pitcher_insight(self, row: pd.Series) -> dict[str, Any]:
        """Build prompt for one bowler-game row and call LLM."""
        prompt = PITCHER_INSIGHT_PROMPT.format(
            pitcher_name       = row.get("pitcher_name", f"Bowler #{row.pitcher_id}"),
            ...
            velo_delta         = row.pace_vs_30d_avg or 0.0,
            play_and_miss_rate         = row.play_and_miss_rate or 0.0,
            dot_ball_percentage           = row.dot_ball_percentage or 0.0,
            stump_line_rate          = row.stump_line_rate or 0.0,
            wide_delivery_rate         = row.wide_delivery_rate or 0.0,
            avg_expected_runs_per_ball_allowed  = row.avg_expected_runs_per_ball_allowed or 0.0,
            boundary_rate_allowed= row.boundary_rate_allowed or 0.0,
            variation_diversity    = row.variation_diversity or 0.0,
