...
========================
Runs the full CricketIQ pipeline end-to-end:
    1. Ingest Ball Tracking (Bronze)
...
def run_pipeline(start: str, end: str, skip_enrich: bool = False) -> None:
    logger.info("=" * 60)
    logger.info("CricketIQ Pipeline — %s → %s", start, end)
    logger.info("=" * 60)

    if not DEMO_MODE:
        # Step 1: Bronze ingestion
        logger.info("[1/4] Ingesting Ball Tracking data...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run full CricketIQ pipeline")
