...
def load_bronze_to_silver(con: duckdb.DuckDBPyConnection) -> None:
    """
    Scan all Bronze Parquet partitions and create the Silver `deliveries` table.
    Uses DuckDB's hive-partitioned Parquet reader for efficiency.
    """
    bronze_glob = str(BRONZE_DIR / "ball_tracking/**/*.parquet")
    ...
    n = con.execute("SELECT COUNT(*) FROM deliveries").fetchone()[0]
    logger.info("Silver deliveries table: %d rows", n)
...
def create_players_table(con: duckdb.DuckDBPyConnection) -> None:
    """
    Build a players lookup from bowler/batsman IDs found in deliveries.
    ...
    SELECT DISTINCT
        ...
        CONCAT('Bowler #', pitcher_id) AS full_name,
        p_throws AS hand,
        'SP' AS position
    FROM deliveries
    ...
    SELECT DISTINCT
        ...
        CONCAT('Batsman #', batter_id) AS full_name,
        stand AS hand,
        'POS' AS position
    FROM deliveries
