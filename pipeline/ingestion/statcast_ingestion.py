...
========================================
Bronze-layer ingestion of International Cricket Ball Tracking data via Cricsheet.

Pulls delivery-by-delivery event data for a date range and writes
...
# Ball Tracking columns we care about — keeps Parquet files lean
KEEP_COLS = [
    "game_pk", "game_date", "over", "inning_topbot",
    "bowler", "batsman",
...
def ingest_date_range(start: str, end: str) -> None:
    """
    Ingest all Ball Tracking deliveries for a date range.

    Writes one Parquet file per day under:
        data/bronze/ball_tracking/game_date=YYYY-MM-DD/part-000.parquet

    Args:
        start: ISO date string (e.g. '2024-07-01')
        end:   ISO date string (inclusive, e.g. '2024-07-07')
    """
    pb.cache.enable()          # cache raw downloads to ~/.Cricsheet/
...
def _ingest_single_day(game_date: str) -> Path:
    """Pull one day of Ball Tracking data and write to Bronze Parquet."""
    out_dir  = BRONZE_DIR / f"ball_tracking/game_date={game_date}"
    ...
    logger.info("Fetching Ball Tracking: %s", game_date)
    try:
        raw: pd.DataFrame = pb.ball_tracking(
            start_dt=game_date,
            end_dt=game_date,
            verbose=False,
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Ingest Ball Tracking data to Bronze layer")
